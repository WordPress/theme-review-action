 ...
 * @version   2.6.1 for parent theme C9 Starter for publication on WordPress.org
		...
		protected function do_plugin_install()
		{
			...
			if ((isset($_GET['tgmpa-install']) && 'install-plugin' === $_GET['tgmpa-install']) || (isset($_GET['tgmpa-update']) && 'update-plugin' === $_GET['tgmpa-update'])) {
				...
				if ($this->is_tgmpa_complete()) {
					echo '<p>', sprintf(esc_html($this->strings['complete']), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'c9-starter') . '</a>'), '</p>';
			...
			if (!empty($desired_slug)) {
				...
				if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
					...
					if (true === $GLOBALS['wp_filesystem']->move($from_path, $to_path)) {
						...
						return new WP_Error('rename_failed', esc_html__('The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'c9-starter') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'c9-starter'), array('found' => $subdir_name, 'expected' => $desired_slug));
					}
				} elseif (empty($subdir_name)) {
					return new WP_Error('packaged_wrong', esc_html__('The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'c9-starter') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'c9-starter'), array('found' => $subdir_name, 'expected' => $desired_slug));
				}
			}
...
if (!class_exists('TGMPA_List_Table')) {
	...
	class TGMPA_List_Table extends WP_List_Table
	{
		...
		{
			echo esc_html__('No plugins to install, update or activate.', 'c9-starter') . ' <a href="' . esc_url(self_admin_url()) . '"> ' . esc_html__('Return to the Dashboard', 'c9-starter') . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
					...
					public function add_strings()
					{
						if ('update' === $this->options['install_type']) {
							...
							if ($this->tgmpa->is_automatic) {
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = __('%1$s installed and activated successfully.', 'c9-starter') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'c9-starter') . '</span><span class="hidden">' . esc_html__('Hide Details', 'c9-starter') . '</span>.</a>';
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = esc_html__('%1$s installed successfully.', 'c9-starter') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'c9-starter') . '</span><span class="hidden">' . esc_html__('Hide Details', 'c9-starter') . '</span>.</a>';
					...
					public function bulk_footer()
					{
						...
						if ($this->tgmpa->is_tgmpa_complete()) {
							...
							$update_actions['dashboard'] = sprintf(
								...
								'<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'c9-starter') . '</a>'
							);
						} else {
